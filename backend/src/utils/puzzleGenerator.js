// backend/src/utils/puzzleGenerator.js
function generatePuzzle(gridSize) {
  ...
  let solvedGrid = Array.from({ length: gridSize }, () => Array(gridSize).fill(0));
  solveSudoku(solvedGrid, 0, 0);

  // Create a puzzle by removing numbers
  let puzzleGrid = JSON.parse(JSON.stringify(solvedGrid)); // Deep copy
  removeNumbers(puzzleGrid, gridSize * gridSize / 2); // Remove ~50% of numbers
